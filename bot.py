...
async def wrapper(ans: Message):
	...
	if response.lower() == 'начать':
		...
		if db.balance1(user_id) >= 75:
			...
			await ans('Недостаточно голды (💰) для вывода средств.У вас на балансе должно быть как минимум 75 голды!💰 Купить голду можно в разделе Баланс')
		...
		if db.qio(user_id) == False   :
			...
			if db.chek2(user_id) == 1:
				await ans("За выполнение всех (10) заданий он получит 700 рублей, которые сразу же сможет вывести")
				...
				for i in range(0,len(qde['donates'])):
					...
					if id == edq:
						...
						if db.plus_balance(user_id, sum) == 1:
							await bot.api.messages.send(
								peer_id=user_id,
								message="Задание пройдено!\nВы получаете награду: 70000🧿и 700💰",
								random_id=bot.extension.random_id())
						else:
							await bot.api.messages.send(
								peer_id=user_id,
